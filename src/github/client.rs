...
impl GitHubClient {
    ...
    pub async fn rank_organizations_by_followers(
        ...
        let stargazers_progress = spinner("Fetching stargazers");
        let stargazer_names =
            fetch_stargazer_names(&self.octocrab, repo_owner, repo_name, &stargazers_progress)
                .await?;
        stargazers_progress.finish_and_clear();

        let memberships_progress = progress_bar(stargazer_names.len(), "Fetching org memberships");
        ...
        .await?;
        memberships_progress.finish_and_clear();

        let total_organizations = organization_names.len();
        let mut organizations = Vec::with_capacity(total_organizations);

        let followers_progress =
            progress_bar(total_organizations, "Fetching organization followers");
        ...
        followers_progress.finish_and_clear();
